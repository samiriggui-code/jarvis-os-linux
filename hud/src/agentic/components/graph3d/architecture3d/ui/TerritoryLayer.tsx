...
function writeCross(cx: number, cy: number, cz: number, arm: number, offset: number, out: Float32Array): void {
  const b = offset;
  out[b] = cx - arm;
  out[b + 1] = cy;
  out[b + 2] = cz;
...
export type TerritoryFrame = {
  positions: Map<string, THREE.Vector3>;
  bounds: { center: THREE.Vector3; radius: number };
  cameraDistance: number;
};

/** Générique — même mécanique que CoreTerritoryLayer, pour n'importe quel nœud CodeMap. */
export function computeTerritoryFrame(
  nodes: CoreL1Node[],
  center: THREE.Vector3 | readonly [number, number, number],
): TerritoryFrame | null {
  if (!nodes.length) return null;
  const c: readonly [number, number, number] =
    center instanceof THREE.Vector3 ? [center.x, center.y, center.z] : center;
  const positions = layoutCoreTerritory(nodes, c);
  const bounds = coreTerritoryBounds(c, positions.values());
  return { positions, bounds, cameraDistance: cameraDistanceForBounds(bounds.radius) };
}

/**
 * Détail L2 (et au-delà) — mêmes croix + fils que CoreTerritoryLayer, mais
 * paramétré (nodes/centre/lod explicites) au lieu de lire CORE en dur, pour
 * fonctionner sur n'importe quel territoire (architecture/, executors/, ...).
 */
export function TerritoryLayer({
  ...
  useFrame(({ camera }) => {
    if (!group.current || !layout || !center) return;
    ...
    const mv = new THREE.Matrix4().multiplyMatrices(camera.matrixWorldInverse, group.current.matrixWorld);
