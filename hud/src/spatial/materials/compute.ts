...
export function materialSpec(material: GlassMaterial = 'regular', mode: SpatialMode = 'night'): MaterialSpec {
  const base = spatialTokens.material[material];
...
export function glassShadows(
  ...
  const mul = spatialTokens.elevation[elevation].shadowMul * spec.shadowStrength;
  const paper = opts?.paperRgb ?? spatialTokens.color.paper;
  const accent = opts?.accentRgb ?? spatialTokens.color.accent;
...
export function elevationTransform(elevation: SpatialElevation): string {
  const e = spatialTokens.elevation[elevation];
