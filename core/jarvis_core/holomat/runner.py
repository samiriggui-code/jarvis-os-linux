...
logger = logging.getLogger("jarvis.holomat.runner")
...
class FaceRunner:
    ...
    def status(self) -> dict[str, Any]:
        return {
            ...
            "waits": self.waits,
            "algo": "opencv_sface" if self.ready else None,
        }
    ...
    async def _load(self) -> bool:
        ...
        logger.info("Holomat : chargement YuNet + SFace en tâche de fond…")
        try:
            ...
            logger.warning(
                "→ python -m jarvis_core.holomat.fetch_models  puis relancer le Core"
            )
            return False
    ...
    async def enroll_add_frame(self, username: str, jpeg_b64: str) -> dict[str, Any]:
        return await self._call("enroll_add_frame", username, jpeg_b64)

    async def enroll_begin(self, username: str) -> None:
        # Pas de CV, mais mute `_enroll` : passer par le verrou quand même.
        await self._call("enroll_begin", username)

    async def enroll_commit(self, username: str, user_id: str) -> dict[str, Any]:
        return await self._call("enroll_commit", username, user_id)
